...
<div className="row px-1 mt-1" style={{width:"102%"}}>
    ...
    borderRight: "1px solid #dcd9d9"
              ...
              <div className="row p-1">
                <div className="col-12 addToCrmLabel" style={{background:`${Qual == 0  || Qual == 1? `#FE8700` :`#489767` }`}}>
            <p className="mb-0 ">QUALIFIED</p>
            </div> 
            </div>
              <div className="row PrECONTACTEDInput ">
                <div className="col-2 pr-0 d-flex justify-content-start align-items-center " ><label className={`btn d-flex align-items-center  ${Qual == 0 ? "themeColorAgency" :   "offBtns"}`} htmlFor={`Qua0${length}`}> <div className="d-flex justify-content-center align-item-center inputBorder"> <input  name={`QUALIFIED${length}`} id={`Qua0${length}`} onClick={(e)=>OnChangeRadio(e,props._id)} value={0}  type={"radio"} className="cursor-pointer d-flex justify-content-center align-items-center" defaultChecked={Qual  === 0  ? true : props.leadQualified === 0  ? true : false}  /></div>?</label></div>
