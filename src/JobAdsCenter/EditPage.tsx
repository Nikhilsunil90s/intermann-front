...
export default function EditReaserch() {
  ...
  const [profile, setProfile] = useState(state) as any;
  const [onClickChangeArea, setOnclickChangeArea] = useState(true);
  ...
  const [addData, setAddData] = useState({
    ...
    adDescription: profile.adDescription, // string
  ...
  const [editorState, setEditorState] = useState(
    EditorState.createWithContent(
      ContentState.createFromBlockArray(convertFromHTML(profile.adDescription))
    )
  ) as any;
  ...
  const onInputChange = (e) => {
    if (e.target.name === "JobFrench") {
      setAddData({ ...addData, adNameFrench: e.target.value });
    }
    if (e.target.name === "JobINRomanian") {
      setAddData({ ...addData, adNameRomanian: e.target.value });
    }
  };

  const SelectOnChange = (e) => {
    if (e.name === "Contry") {
      setAddData({ ...addData, adCountryMarket: e.value });
    } else if (e.name === "importance") {
      setAddData({ ...addData, adImportance: e.value });
    } else {
      const ClF = e.map((el) => el.value);
      setAddData({ ...addData, clients: ClF });
    }
  };
        ...
        <div className="row">
          ...
          >
            <div className="row">
              ...
              <div className="col-4 mt-1">
                <label className="Form-styling">
                  link a client to this ad (client selector){" "}
                </label>
                {clients.length > 0 ? (
                  <Select
                    options={clients}
              ...
              <div className="col-12 d-grid mt-1">
                ...
                >
                  {onClickChangeArea ? (
                    <div
                      className="jobCardContentScroll p-1"
                      style={{ height: "250px" }}
                      onClick={() => {
                        setOnclickChangeArea(false);
                      }}
                    >
                      {parse(profile.adDescription)}
                    </div>
                  ) : (
                    <Editor
