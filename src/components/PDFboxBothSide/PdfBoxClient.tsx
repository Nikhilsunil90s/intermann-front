...
function PDFBoxClient({props,value,updated}){
  ...
  useEffect(() => {
    ...
    setDocumentList(props?.clientDocuments.filter((el) => el.folderName == UploadName));
  },[props]);
 ...
 useEffect(()=>{
  ...
  const fetchClient =(props_id: any) => {
     fetch(API_BASE_URL + `getClientById/?clientId=${props_id}`, {
      ...
      .then(resp => {
        if(resp.status){
          resp.data.map((el)=>{
            setDocumentList(el.clientDocuments.filter((el) => el.folderName == UploadName))
            setLinkList(el?.clientLinks.filter((props) => props.folder == UploadName));
            setProfile(el)
            value(el)
  ...
  const onTabClick = (e,index: any) => {
    ...
    setLinkList(profile?.clientLinks.filter((props) => props.folder == UploadName));
    setDocumentList(profile?.clientDocuments.filter((el) => el.folderName === UploadName));
  };
...
// Notify //
        ...
        let Data={
         ...
         linkId:Id,
          ...
          fetch(API_BASE_URL + "removeClientLink", {
             ...
             .then(reD => reD.json())
             .then(resD =>{toast.success(resD.message);setDeleteStatus(true);updated(true)})
  ...
  const onDriveLinkChange=(e)=>{
    ...
    if(e.target.name =="DriveLinkSubmit"){
      ...
      if(Check){
        LinktoDrive(Data).then((resD)=>{toast.success(resD.message);setDeleteStatus(true);updated(true)})
      }else{
