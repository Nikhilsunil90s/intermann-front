...
function PDFBoxCandidate({ props, value }) {
  ...
  useEffect(() => {
    ...
    setDocumentList(
      props.candidatDocuments.filter((el) => el.folderName == UploadName)
    );
  }, [props]);
  ...
  const onTabClick = (e, index: any) => {
    ...
    setDocumentList(
      profile?.candidatDocuments.filter((el) => el.folderName == UploadName)
    );
  };
  ...
  const fetchCandidat = async (candidatId: any) => {
    await fetch(API_BASE_URL + `getCandidatById/?candidatId=${candidatId}`, {
      ...
      .then((resp) => resp.json())
      .then((respData) => (respData.status ? dataHandle(respData.data) : null))
      .catch((err) => err);
  };
  ...
  const dataHandle = (el) => {
    ...
    setDocumentList(
      el.candidatDocuments?.filter((el) => el.folderName == UploadName)
    );
  ...
  const FilesUploads = (file) => {
    ...
    axiosInstance
      .post("uploadCandidatDocuments", formdata, {
        headers: {
  ...
  const deleteCandidatDocument = async (
    ...
    return await fetch(
      API_BASE_URL +
        `deleteDocument/?documentId=${docId}&documentName=${docName}&candidatId=${candidatId}`,
      {
        method: "GET",
        headers: headers,
      }
    ...
    await deleteCandidatDocument(docId, docName, profile._id)
      .then((resData) => {
        if (resData.status) {
          ...
          setDocumentList([
            ...documentList.filter((docN) => docN.documentName !== resData.doc),
          ]);
        } else {
                    ...
                    <div className="row CardClassDownload mt-1 mx-0">
                      ...
                      <div className="col-6 text-center">
                        ...
                        <button
                          ...
                          className="btnDownload"
                          onClick={(e) => ViewDownloadFiles(Link.link)}
                        >
                          <img
                            src={require("../../images/dowBtn.svg").default}
                          />
