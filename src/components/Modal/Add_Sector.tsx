...
function SectorModal() {
  ...
  const checkSectorExistence = async (sector: String) => {
    return await fetch(
      API_BASE_URL + "checkSectorExistence/?sector=" + sector,
      {
        method: "GET",
        headers: {
                ...
                jobsList.map((job, index) => (
                  <div key={index} className="jobs">
                    <input
                      ...
                      type="text"
                      className="form-control"
                      placeholder={"Enter Job Name in Sector -  " + sector}
                      onChange={(e) => {
                        handleJobInputChange(e, index);
                      }}
                    />
                    <br></br>
                    {jobsList.length - 1 === index && (
                      <button
                        ...
                        onClick={(e) => handleJobAdd(job.jobName)}
                      >
                        Save & Add Other Job
                      </button>
                    )}
