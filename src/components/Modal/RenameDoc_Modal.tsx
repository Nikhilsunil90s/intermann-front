...
function RenameDoc({ props, closepreModal, status }) {
  ...
  const renameCandidatDocument = async () => {
    ...
    if (props[2] === "LinkEdit") {
      ...
      return await fetch(
        API_BASE_URL +
          `renameDocument/?documentId=${props[0]}&newName=${NewName}&candidatId=${props[2]}`,
        {
          method: "GET",
          headers: headers,
        }
        ...
        .then((reD) => reD.json())
        .then((resD) =>
          resD.status === true ? RenameMoved() : notifyCandidatMovedError()
        )
        .catch((err) => err);
    }
  };
          ...
          <div className="modal-content" style={{ width: "670px" }}>
            ...
            <div className="modal-footer">
              <button
                ...
                onClick={renameCandidatDocument}
              >
                {props[2] === "LinkEdit"
                  ? "Save Display Name."
                  : "Save New Name."}
